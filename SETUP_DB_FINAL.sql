-- STARTFLIX PRO - COMPLETE DATABASE SETUP (FINAL VERSION)
-- Executar este script no editor SQL do Supabase.
-- Garante integração 100% com o App (Dashboard Admin, Login, M3U, Watchlist, Pagamentos, etc.)
...
-- 1.2 LISTAS M3U PADRÃO (Atribuídas automaticamente)
...
-- 1.3 ADMINS (Lista explícita para RLS)
...
-- 1.4 PERFIS DE USUÁRIO (Tabela Principal)
CREATE TABLE IF NOT EXISTS public.profiles (
    id uuid PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,
    username text,
    email text,
    password text, -- Referência visual apenas
    full_name text,
    role text DEFAULT 'user',
    
    -- Configuração do Cliente
    m3u_url text, -- Lista atribuída
    is_active boolean DEFAULT true,
    ...
    -- Credenciais do App Selecionado
...
-- =====================================================================
-- 2. TABELAS DE CURSOS FUNCIONAIS
-- =====================================================================

-- 2.1 CONTAS DE MÍDIA (Estoque e TVs Gerenciadas)
CREATE TABLE IF NOT EXISTS public.media_accounts (
    id uuid DEFAULT gen_random_uuid() PRIMARY KEY,
    user_id uuid REFERENCES public.profiles(id) ON DELETE SET NULL, -- Null = Em estoque
    provider_name text,
    username text,
    password text,
    dns text,
...
-- 2.2 TVS DO CLIENTE (Atribuição Direta)
...
-- 2.4 LISTA DE FAVORITOS (Watchlist)
...
-- 5.1 Criar Perfil ao Criar Usuário (RPC)
...
-- 5.2 Auto-Atribuir Lista M3U Padrão
...
-- 5.3 Auto-Atribuir TV do Estoque (Opcional, mas mantido)
...
-- 5.4 Atualizar Credenciais (Admin)
...
CREATE POLICY "Public can view images" ON storage.objects FOR SELECT TO public USING (bucket_id = 'app-images');
...
SELECT 'Setup Finalizado com Sucesso!' as status;
